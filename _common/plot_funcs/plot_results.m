function [handles,ospa_vals,ospa2_vals]= plot_results(model,truth,meas,est,varargin)
  % --- Input Parser
    pRaw = inputParser;
    addParameter(pRaw,'plot_flag', true, @islogical);
    addParameter(pRaw,'save_plot_flag', true);
    parse(pRaw, varargin{:});
    plot_flag = pRaw.Results.plot_flag;
    save_plot_flag = pRaw.Results.save_plot_flag && plot_flag ;
    folder_path = 'results_demo_tmp/';
    ...
    if plot_flag
        ...
        if save_plot_flag
            print_with_catch([folder_path,'grounth_truth.png'],'-dpng');
        end
    end
    if plot_flag
...
%         legend([hline2 hline1 hlined],'Estimates          ','True tracks','Measurements');
        ...
        if save_plot_flag
            print_with_catch([folder_path,'estimates-xy.png'],'-dpng');
        end
    end
    %plot error - OSPA
    ...
    if plot_flag
        ...
        xlabel('Time');
        if save_plot_flag
            print_with_catch([folder_path,'ospa.png'],'-dpng');
        end
    end
    %plot error - OSPA^(2)
    order = model.ospa.p;
    cutoff = model.ospa.c;
    win_len= model.ospa.win_len; %truth.K;
    ...
    ospa2_vals = cell2mat(ospa2_cell)';
    if plot_flag
...
%         legend(windowlengthlabels,'interpreter','latex');
        ...
        xlabel('Time','interpreter','latex');
         if save_plot_flag
            print_with_catch([folder_path,'ospa2.png'],'-dpng');
        end
...
%         legend(gca,'True','Estimated');
        ...
        if save_plot_flag
            print_with_catch([folder_path,'card.png'],'-dpng');
        end
