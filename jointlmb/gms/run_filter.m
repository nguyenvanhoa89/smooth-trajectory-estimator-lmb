...
for tabsidx=1:length(tt_lmb_update)
    ...
    [mtemp_predict,Ptemp_predict]= model.filter.predict_func(model,tt_lmb_update{tabsidx}.m,tt_lmb_update{tabsidx}.P);        %kalman prediction
...
if filter.gate_flag
    for tabidx=1:length(tt_predict)
        tt_predict{tabidx}.gatemeas= model.filter.gate_func(meas.Z{k},filter.gamma,model,tt_predict{tabidx}.m,tt_predict{tabidx}.P);
    end
else
    for tabidx=1:length(tt_predict)
